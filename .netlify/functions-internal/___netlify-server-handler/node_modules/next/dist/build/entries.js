...
async function collectAppFiles(appDir, pageExtensions) {
    const validFileMatcher = (0, _findpagefile.createValidFileMatcher)(pageExtensions, appDir);
    ...
    const allAppFiles = await (0, _recursivereaddir.recursiveReadDir)(appDir, {
        pathnameFilter: (absolutePath)=>validFileMatcher.isAppRouterPage(absolutePath) || validFileMatcher.isRootNotFound(absolutePath) || validFileMatcher.isAppLayoutPage(absolutePath),
...
async function collectPagesFiles(pagesDir, pageExtensions) {
...
function createRelativeFilePath(baseDir, filePath, prefix) {
    const privatePrefix = prefix === 'pages' ? 'private-next-pages' : 'private-next-app-dir';
    return (0, _path.join)(baseDir, filePath.replace(new RegExp(`^${privatePrefix}/`), `${prefix}/`));
}
function processPageRoutes(mappedPages, baseDir) {
    const pageRoutes = [];
    const pageApiRoutes = [];
...
function processAppRoutes(mappedAppPages, baseDir) {
    const appRoutes = [];
...
function processLayoutRoutes(mappedAppLayouts, baseDir) {
    const layoutRoutes = [];
